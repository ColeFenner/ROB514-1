...
# YOUR CODE HERE
    ...
    transformed_x = mat @ x_axis
    transformed_y = mat @ y_axis
    sx = np.linalg.norm(transformed_x[:2])
    sy = np.linalg.norm(transformed_y[:2])
    return sx, sy
...
# YOUR CODE HERE
    ...
    transformed_point = mat @ origin
    dx, dy = transformed_point[0], transformed_point[1]
    return dx, dy
...
# YOUR CODE HERE
    ...
    x_rotated_axis = mat @ x_axis
    y_rotated_axis = mat @ y_axis
    return x_rotated_axis, y_rotated_axis
...
# YOUR CODE HERE
    x_rotated_axis, _ = get_axes_from_matrix(mat)
    theta = np.arctan2(x_rotated_axis[1], x_rotated_axis[0])
    return theta
...
# YOUR CODE HERE
    x_rotated_axis, y_rotated_axis = get_axes_from_matrix(mat)
    if not np.isclose(np.linalg.norm(x_rotated_axis), 1) or not np.isclose(np.linalg.norm(y_rotated_axis), 1):
        if b_print:
            print("One of the transformation axes is not a unit vector.")
        return False
    
    # Step 2: Check orthogonality (dot product should be close to 0)
    if not np.isclose(np.dot(x_rotated_axis, y_rotated_axis), 0):
        if b_print:
            print("Transformation axes are not orthogonal.")
        return False

    # Step 3: Check that the inverse is equal to the transpose
    if not np.allclose(mat @ mat.T, np.eye(mat.shape[0])):
        if b_print:
            print("Matrix is not orthonormal (inverse is not the transpose).")
        return False
...
# YOUR CODE HERE
    ...
    cross_product = np.cross(x_rotated_axis, y_rotated_axis)
    return cross_product[2] <= 0  # Returns True if mirrored (z component is not positive)
...
# YOUR CODE HERE
    x_rotated_axis, y_rotated_axis = get_axes_from_matrix(mat)
    dot_product = np.dot(x_rotated_axis, y_rotated_axis)
    if np.isclose(dot_product, 0):
        return True  # Angles are preserved (orthogonal)
    else:
        return False  # Angles are not preserved
...
# YOUR CODE HERE
    pts = np.ones((3, n_pts))
    pts[0, :] = np.cos(ts)  # x values
    pts[1, :] = np.sin(ts)  # y values
...
# YOUR CODE HERE
    pts_moved = mat @ pts  # Matrix multiplication to get new circle points
...
def example_order_matters():
    ...
    seq_trans_rot = [{"type":"translate", "dx": 1, "dy": 2},
                     {"type":"rotate", "theta": np.pi/4.0}]
    mat = make_matrix_from_sequence(seq_trans_rot)
...
# YOUR CODE HERE
    seq_trans_rot_scl = [{"type": "translate", "dx": 1, "dy": 2},
                          {"type": "rotate", "theta": np.pi / 4.0},
                          {"type": "scale", "sx": 0.5, "sy": 2.0}]
    mat = make_matrix_from_sequence(seq_trans_rot_scl)
...
def example_weird_geometry():
    ...
    # TODO: Make seq_mirrored so that the x axes are flipped, and the x axis is twice as big as it was before.
...
# YOUR CODE HERE
    seq_skew = [
        {"type": "rotate", "theta": np.pi / 4},  # Rotate by 45 degrees
        {"type": "scale", "sx": 1.5, "sy": 0.5},  # Non-uniform scaling
        {"type": "translate", "dx": 2.5, "dy": 2.5}  # Translating to (2.5, 2.5)
    ]
...
# YOUR CODE HERE
    ...
    pts_circle_lower_left_origin = pts_circle + np.array([[1], [1], [0]])  # Shift circle points to lower left
    pts_zigzag_lower_left_origin = pts_zigzag + np.array([[1], [1], [0]])

    rotation_matrix = make_rotation_matrix(np.pi / 2)
    pts_circle_x_up = rotation_matrix @ pts_circle
    pts_zigzag_x_up = rotation_matrix @ pts_zigzag

    pts_circle_x_up_lower_left_origin = pts_circle_x_up + np.array([[1], [1], [0]])
    pts_zigzag_x_up_lower_left_origin = pts_zigzag_x_up + np.array([[1], [1], [0]])
